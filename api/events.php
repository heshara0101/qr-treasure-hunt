...
function handleAddTask($input) {
    ...
    $correct_answer = $input['correct_answer'] ?? null;   // may be array
    $qr_value       = $input['qr_value']       ?? null;
    $qr_location    = $input['qr_location']    ?? null;   // 🔹 NEW

    // make sure correct_answer is a STRING
    if (is_array($correct_answer)) {
        $correct_answer = reset($correct_answer);
        // or: $correct_answer = implode(',', $correct_answer);
    }
    ...
    // 🔹 added qr_location column + placeholder
    $stmt = $db->prepare("INSERT INTO tasks 
        (level_id, task_number, type, question, options, correct_answer, qr_code_value, qr_location) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param(
        "iissssss",
        $level_id,
